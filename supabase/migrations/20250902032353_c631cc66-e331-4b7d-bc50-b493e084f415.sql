...
ADD COLUMN payment_method text DEFAULT 'mercadopago';

-- Update the notification trigger to handle Cash/POS orders
...
BEGIN
  ...
  -- For order_and_pay mode with Cash/POS: create new_order notification when order is created
  IF TG_OP = 'INSERT' AND operation_mode = 'order_and_pay' AND NEW.payment_method = 'cash_pos' THEN
    -- Get order details with items
    SELECT 
      STRING_AGG(
        oi.quantity || 'x ' || mi.name || ' ($' || (oi.unit_price * oi.quantity)::numeric(10,2) || ')',
    ...
    IF order_details IS NOT NULL AND order_details != '' AND restaurant_id IS NOT NULL THEN
      ...
      VALUES (
        ...
        'Mesa ' || COALESCE(table_number::text, '0') || ' pidió: ' || order_details || ' - Total: $' || COALESCE(NEW.total_amount::numeric(10,2)::text, '0.00') || ' (Pago en efectivo/posnet)'
      );
    END IF;
  END IF;
  
  -- CRITICAL FIX: Only create payment notification when payment_status actually changes from non-paid to 'paid'
  IF TG_OP = 'UPDATE' AND 
     OLD.payment_status IS DISTINCT FROM 'paid' AND 
     NEW.payment_status = 'paid' THEN
